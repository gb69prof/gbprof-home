...
s=s.replace('href="lezioni.html"','href="#lezioni"').replace('href="strumenti.html"','href="#strumenti"').replace('href="/Spazi/"','href="#spazi"')
...
panes=''
for name in ['lezioni','strumenti','spazi']:
 page=(r/'Spazi/index.html' if name=='spazi' else r/f'{name}.html').read_text()
 body=re.search(r'<body>(.*?)</body>',page,re.S)[1]
 body=body.replace('href="index.html#porte"','href="#porte"').replace('href="index.html"','href="#top"')
 if name=='spazi':
  body=body.replace('href="/#porte"','href="#porte"').replace('href="/"','href="#top"')
  style=re.search(r'<style>(.*?)</style>',page,re.S)[1]
  panes+=f'<style>{style}</style>'
...
js=js.replace("didattica:'lezioni.html'","didattica:'#lezioni'").replace("strumenti:'strumenti.html',",'').replace("laboratorio:'/Arte/'", "laboratorio:'#spazi'")
route='''\n(() => {const home=document.getElementById('home-view'); const names=['lezioni','strumenti','spazi']; function displayPage(){const key=location.hash.slice(1),selected=names.includes(key);home.hidden=selected;names.forEach(n=>{const el=document.getElementById(n);el.style.display=n===key?'block':'none';});if(selected){window.scrollTo(0,0);const h=document.querySelector('#'+key+' h1');h.setAttribute('tabindex','-1');h.focus({preventScroll:true});}else if(['top','porte'].includes(key)){document.getElementById(key).scrollIntoView();}}window.addEventListener('hashchange',displayPage);displayPage();})();'''
